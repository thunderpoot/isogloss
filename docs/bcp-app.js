...
function highlightTag(input) {
    ...
    const regex = /^(?:(?<grandfathered>(?<irregular>en-GB-oed|i-ami|i-bnn|i-default|i-enochian|i-hak|i-klingon|i-lux|i-mingo|i-navajo|i-pwn|i-tao|i-tay|i-tsu|sgn-BE-FR|sgn-BE-NL|sgn-CH-DE)|(?<regular>art-lojban|cel-gaulish|no-bok|no-nyn|zh-guoyu|zh-hakka|zh-min|zh-min-nan|zh-xiang))|(?:(?:(?<language>[a-z]{2,3})(?:-(?<extlang1>([a-z]{3}))){0,3})|(?<language4>[a-z]{4})|(?<language5>[a-z]{5,8}))(?:-(?<script>[a-z]{4}))?(?:-(?<region>[a-z]{2}|[0-9]{3}))?(?:-(?<variant>([a-z0-9]{5,8}|[0-9][a-z0-9]{3})))*?(?:-(?<extensions>[a-wy-z0-9]-(?:[a-z0-9]{2,8})+))*?(?:-x-(?<privateuse>([a-z0-9]{1,8}(?:-[a-z0-9]{1,8})*))?)?|x-(?<privateuse2>([a-z0-9]{1,8}(?:-[a-z0-9]{1,8})*)))$/i;
    ...
    if (privateuse2) {
        highlightedHtml += `<span class="hyphen">-</span><span class="private-use">x-${privateuse2}</span>`;
    }
...
function processTag() {
    ...
    if (parsedTag.extLangs.length > 0) {
        ...
        html += `<div class="section extended-language"> <strong>Extended Languages:</strong> <ul>${extendedLanguagesHtml}</ul> </div>`;
    }
    // Process Script
    if (parsedTag.script) {
